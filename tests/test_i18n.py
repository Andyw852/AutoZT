...
def _run(code, lang=None):
    env = dict(os.environ)
    env.pop("AUTOZT_LANG", None)
    env.pop("LANG", None)
    if lang:
        env["AUTOZT_LANG"] = lang
...
def test_default_is_chinese_fallback():
    out = _run("import sys;sys.path.insert(0,'.');"
               "from autozt import i18n;print(i18n.t('中文','English'))")
    assert out == "中文"


def test_en_switches_to_english():
    out = _run("import sys;sys.path.insert(0,'.');"
               "from autozt import i18n;print(i18n.t('中文','English'))", lang="en_US.UTF-8")
    assert out == "English"


def test_hint_table_falls_back_and_translates():
    code = ("import sys;sys.path.insert(0,'.');"
            "from autozt import preflight as pf;"
...
def test_lang_env_alone_is_enough():
    out = _run("import sys;sys.path.insert(0,'.');"
               "from autozt import i18n;print(i18n.is_en())", lang="en")
    assert out == "True"


def test_diag_maps_known_phrases_only_when_english():
    code = ("import sys;sys.path.insert(0,'.');"
            "from autozt import i18n;"
