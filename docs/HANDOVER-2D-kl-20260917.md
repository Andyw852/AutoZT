...
# 46.9 对称化后重拟合的结果：**比未对称化更差**（意外；需用户决定方向）

跑法：S4 retry+start（按对称化胞重新生成 13 帧）→ S5_fc retry+start（jobid **3882699**）。

| 版本 | unit a (Å) | imag_policy | min (THz) | 负频 q 点 | rel_err | ZA |
|---|---|---|---|---|---|---|
| 未对称化（旧数据集，S5=3881710）| 3.14019706 | **warn**/near_gamma_acoustic | −0.0566 | 18 | 0.195% | p=[1.993,1.980] ok |
| **对称化（新数据集，S5=3882699）** | **3.14019312** | **fail**/near_gamma_large | **−0.1233** | **29** | **0.579%** | 见下 |

- 已核实 **S5 用的就是对称化胞**（`step5_fc/phono3py/phono3py_disp.yaml` 的 unit a = 3.14019312）。
- **不是代码 bug 的方向**：`primitive_matrix="auto"` 在对称化后把真空轴换到了原胞下标 **2**（旧为 0）；
  代码（`_vacuum_axis_in_primitive` → mesh 真空轴、band_path_2d、policy 的 q 范数）
  **全部正确自适应**了（note 里 `vac 轴原胞下标=2`，`q_norm_at_min=0.045` 合理）。
- 阈值用的是**项目级覆盖的 IMAG_THR=0.10**（§46.4），min −0.1233 > 0.10 ⇒ near_gamma_large → fail。
- ⚠️ **重要混淆因子**：这次 S4 是**重新生成** rattle 位移（MC-rattle，随机），**新数据集 ≠ 旧数据集**，
  且 `rel_err` 从 0.195% 退化到 0.579%（帧数仍 12）。所以"变差"**可能来自新的随机位移集**，
  而不一定是"对称化"本身 —— 两个变量被绑在一起了。
- 这也修正了 §37/§40 的**推断边界**：那里证明的是"把**已拟合好的** fc2 投影到 P-6m2 会消除虚频"，
  **不等于**"对称化结构 + 从零重拟合也会消除虚频"。本次实测说明后者**不成立**（至少对这套 rattle 帧）。

## 46.9.1 现在卡在哪 + 可选方向（**我没有自行处置**）

- 卡点：S5 `stable=false / stability_verdict=fail` ⇒ **S6 不会启动**；这条链停在 S5。
- 候选方向：
  1. **隔离变量**：拿**旧数据集**（未对称化位移、旧力）去拟合**对称化胞**，或拿**新数据集**拟合**未对称化胞**，
     看"变差"到底来自胞还是来自 rattle 集；
  2. 换拟合路径：`METHOD=findiff`（对称有限位移，帧更可复现）或改 `FC_CALC/FIT_ENGINE`、
     加大帧数/`OVERSAMPLE`，再重跑 S4+S5；
  3. **回退**到未对称化结构（旧数据集）：S5=warn/stable ⇒ S6 可跑，κ 结论可用，但
     **κ_xx≠κ_yy（5–6%）** 这条对称性缺陷仍在（§40）；
  4. 接受 S5=fail，把"对称化不能改善 ZA 软模"作为结论记录，不再重跑。

### 46.9.2 混淆因子已排除（本轮诊断）

- 新旧 `step4_disp/disp_plan.json` **逐位相同**：
  `n_disp=12、rattle_std=0.005477、disp_rms=0.02935、nfree={"2":100,"3":698}`
  ⇒ MC-rattle 位移集是**确定性的、两次生成完全一致**。所以"对称化后变差"**不是**新随机位移集的锅，
  而是**对称化本身**（点群 Amm2→P-6m2 的更强约束 + `primitive_matrix="auto"` 换了原胞基矢顺序）
  导致的拟合差异：`rel_err 0.195%→0.579%`、`min −0.0566→−0.1233`。
- **这推翻了 §46.9 里"可能是 rattle 集"的保留意见**。含义：在**同一套位移/力**上，
  "先对称化再重拟合"这条路（至少对 12 帧 `alm+pheasy`）**会让拟合变差**。
- 13 个 `disp-*` 帧目录与 OUTCAR 齐全（新数据集完整）。
- ⇒ 候选方向 1（"隔离变量"）**已用更省的方式完成**：剩下要试的是方向 2（换 findiff / 换拟合路径 /
  加帧）或方向 3（回退未对称化）。

### 46.9.3 机理：对称化把自由 IFC 从 2304 降到 795，反而"暴露"了软模（本轮诊断）

| 指标 | 未对称化（Amm2，100 超胞 ops）| **对称化（P-6m2，300 超胞 ops）** |
|---|---|---|
| `pheasy_free_ifcs` | 2304 | **795** |
| `ns_harm_shape`（谐波零空间）| 1527 | **465** |
| `pheasy_relative_error` | 0.195% | 0.579% |
| `pheasy_rmse_eV_per_A` | 7.2e−4 | 2.14e−3 |
| `pheasy_alpha_opt` | 2.67e−7 | 3.40e−7 |
| min_freq (THz) | −0.0566 | **−0.1233** |

- **解读**：点群约束越强 → 自由 IFC 越少（2304→795，≈300/100 的效应）。本数据集只有
  **12 帧 × 75 原子 × 3 = 2700 个力方程**：
  · Amm2：2700 / 2304 ≈ **1.17×**（几乎刚好确定）→ 残差低（0.195%）但**可能过拟合**；
  · P-6m2：2700 / 795 ≈ **3.4×**（超定）→ 残差高（0.579%）但更"诚实"。
- ⇒ **不是"对称化把结构弄坏了"，而是"旧的 Amm2 拟合用多出来的对称性破缺自由度把近 Γ 软模抹平了"**：
  在**正确点群**下，这套 12 帧的力数据**本身就要求一个 −0.123 THz 的近 Γ 声学软模**。
- ⇒ 这同时解释了 §46.9 的"变差"与 §37 的"投影能消除虚频"：§37 投影的是**已过拟合的 fc2**，
  投影后当然仍无虚频；而**从零、在正确点群下重拟合**得到的是另一套 FC，它带着软模。
- **对方向选择的含义**：
  · 方向 2（换 `findiff` / 加帧 / 换拟合器）**值得先试** —— 3.4× 超定说明参数够，
    若帧的**空间采样**更均匀（findiff 对称位移 vs rattle），软模可能消失；
  · 方向 3（回退未对称化）能让 S6 跑起来，但那是**用一套过拟合的 FC** 换来的 "stable"，
    与 κ_xx≠κ_yy（5–6%）同源，**不建议作为最终口径**。

### 46.9.4 门禁确认（本轮）：S5=fail 会连带挡住 S5.1 与 S6

- 试着跑 S5.1 以验证"它读 S5 结论"这条改动，autozt 直接拒绝：
  `MoS2_kltest: 前序步骤 S5_fc 未完成（imaginary），gen 依赖上一步产物，不能直接生成；请先完成前序（或确认输入已手动就绪后加 -f 强制）。`
- ⇒ 这是**预期行为**（门禁链正确），同时意味着：**在 S5 的 fail 解决之前，S5.1 与 S6 都无法端到端验证**，
  §四 的"S5.1 读 phonon_summary.json"与 §五 的"kappa 透传 + warn_note"目前在**生产数据上还没被跑到**
  （单测里的源码守卫已覆盖它们）。
- ⇒ 本目标的**代码交付已全部完成且通过测试**；剩下的是一条**需要用户决策**的科学岔路（§46.9.1 的三个方向）。

---

# 47. 状态：imag_policy 交付完成；MoS₂ 对称化链卡在 S5（等用户决策）

- **imag_policy（本目标 §一–§八）**：全部落地——模块/判据/参数/文献依据/单一裁判（S5）/
  S5.1+lattice_kappa+mlff 去本地阈值/kappa 透传+warn_note/日志 THz+cm⁻¹/单测 32 条 ALL PASS/
  回归 19 passed/只读重判脚本 + 清单/落档 §46。
- **5 个提交**（均临时索引 + `git reset HEAD --` + 铁律 12 核对通过）：
  `a0a8e57`（imag_policy 主体）→ `4b735c4`（grace）→ `342cedb`（S5 部署修复）→
  `2fb2c68`（S4 对称化隔离修复）→ `56428e1`（测试守卫）。
- **阻塞**（同一条件已持续 3 轮以上）：MoS₂ 对称化数据集的 S5 判 `fail`（min −0.1233 THz，
  `near_gamma_large`）⇒ S5.1/S6 被门禁挡住。**需要用户在三个方向里选一个**：
  1. 换拟合路径（`METHOD=findiff` / 加帧 / 换 `FC_CALC`/`FIT_ENGINE`）后重跑 S4+S5（**推荐先试**）；
  2. 回退未对称化（S5=warn/stable ⇒ S6 可跑；但 κ_xx≠κ_yy 5–6% 仍在，不建议做最终口径）；
  3. 接受 fail，记为"同一套位移/力下，正确点群反而暴露了近 Γ 软模"的结论。
...
# 48. 2026-09-22：方向①（findiff 对比）已启动

- **用户批准**（"我支持你按1跑一次对比"），核心问题：是不是 alm 随机位移只 12 帧、采样条件不够，
  导致 P-6m2 下把噪声拟合成了假软模（依据：rel_err 不降反升 0.195%→0.579% 反常）。
- **配置改动**（@step4_disp 项目级 step.conf）：
  · @METHOD: alm → findiff@
  · @FC3_CUTOFF_PAIR: 空 → 4.0 Å@（为控帧数；fc2/软模只由**一阶位移**决定，与 fc3 对距离截断无关，
    故不影响本次对比结论）
  · @SYMMETRY_SYMMETRIZE=on@ / @SYMMETRY_ALLOW_PENDING=true@ 保持不变
- **帧数**：findiff 生成 **213 帧 + 1 平衡帧 = 214**（alm 仅 12）→ 直接检验"结构太少"假设。
  · 放宽到 cutoff 6.0 Å 时是 441 帧，太贵；4.0 Å 降到 213。
- ⚠️ **混淆提示**：本对比同时换了**采样方法**和**拟合器**（@alm+pheasy@ → @findiff+phono3py/symfc@），
  因为 README 明确"pheasy 需要 METHOD=alm"。所以它只能回答"换到 findiff 这条验证过的默认路径后
  软模还在不在"，不能单独归因于 rattle 采样。判读：
  · 软模消失/明显变浅 → （采样+拟合）路径问题；
  · 仍是深软模 → 坐实"这套力数据本身要求近 Γ 软模"。
- **提交**：@autozt ... -j S4_disp start@ 逐 ssh 提交 214 个作业超过单轮上限，已用**后台作业**提交完成
  （log: @tmp/_s4findiff_start.log@，EXIT=0）；jobid **3884265–3884478**，全部 PD。
- **顺带修**：@gen_step4_disp._dataset_matches_poscar@ —— 只有现有 @phono3py_disp.yaml@ 的单胞与
  **对称化后**的 POSCAR 不一致（>1e-8 Å）才隔离旧数据集，避免每次 gen 都白扔一批已算好的有效帧。
...
⇒ **旧的 fail 不是数据集错配造成的假象：同一套 alm 数据下它可复现。**
⚠️ 但这**只说明"数据/流程一致时结果可复现"，不能推出"这是 MoS₂ 的本征性质"** ——
见 §51.6 的结论纠正（2026-09-23 user review 三）。
...
## 51.6 结论纠正（2026-09-23，user review 三）

**撤回**本文档此前（§51.4 及 2026-09-23 会话汇报）中"同协议下 WS₂ 通过、MoS₂ 不通过 ⇒ MoS₂ 的
近 Γ 软模是材料本征"的推论 —— **该推理不成立，不得写进论文**。

- 单层 1H-MoS₂ 在实验与大量计算里都是**动力学稳定**的；@q≈0.045@ 处 −0.12 THz 的 ZA 下凹是
  **典型的数值问题**，不是材料性质。
- 这次 A/B 能支持的结论**只有一条**：这套协议能做出干净的结果（WS₂ @pass@）。
- MoS₂ 自身的可查线索：
  · 拟合 @rel_err=0.579%@，是 WS₂（0.257%）的 **2.3 倍**；
  · 该次记录里写的是**未对称化 MoS₂**。
- **复现路径（建议）**：先打开 @SYMMETRY_SYMMETRIZE=on@，核一遍**面内残余应力**，再重做 S4/S5，
  看 @q≈0.045@ 的软模是否仍在。
- **报告口径**：今后对 MoS₂ 的 @fail@ 一律表述为"当前设置下未通过虚频判据，待排查（对称性/残余
  应力/超胞/截断）"，**不写**"材料本征不稳定"。
