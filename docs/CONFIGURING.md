...
| 4 | the fit/relax stage dies silently: no traceback, no products | the template activates a conda env or conda.sh path that only exists on the source cluster (e.g. atomate2_p_a and /public/home/<user> on a machine that has mace-gpu) | point CONDA_SH / CONDA_ENV in that step's step.conf at the target cluster's environment (values belong in the cluster's setting file) |
