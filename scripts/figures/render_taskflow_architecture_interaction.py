...
def draw_interaction_flow(d, x, y, w, h):
    ...
    cards = [
        ...
        (2, "Configure", "initialize -> skill -> HPC (optional)", ["tf -p MAT init", "autozt skills"], "gold", "$"),
        (3, "Start / Advance", "generate inputs -> submit -> advance", ["tf -p MAT start", "autozt start"], "teal", "$"),
        (4, "Monitor / Inspect", "observe changes -> inspect -> report", ["autozt summary --diff", "tf -p MAT status"], "blue", "$"),
    ]
