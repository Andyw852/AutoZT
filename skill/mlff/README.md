# mlff —— 随机位移法 MLFF 训练（MACE 势函数产出流水线）

> 技能名 `mlff`；`desc: 随机位移法MLFF训练`。
...
`convergence_history.json`、以及一行可直接填进 `kl-mlff-cpu`/`phonon-mlff-cpu` 的
...
autozt -tt mlff init             # 建项目设置（自动把技能模板拷进 project_setting）
autozt -tt mlff -p Si start      # 一键推进（会先 gen S1 再 sbatch）
# S1 算完后：
autozt -tt mlff -p Si start      # 继续推进 S2→S3→S4→S5（S5 一次交 ~25 个 12 核作业）
autozt -tt mlff -p Si start      # S5 全部 OK 后继续 S6→S7→S8
autozt -tt mlff -p Si start      # S8 pass 后 S9 发布
```
...
autozt -tt mlff init
autozt -tt mlff -p MoS2 conf --set params.FUNC=pbe-d3   # 需要色散修正的 2D 层状材料
autozt -tt mlff -p MoS2 start
...
```bash
autozt -tt mlff -p Si conf --set params.GENERATION=1
autozt -tt mlff -p Si -j 4 retry    # ★ 用 retry 别用 rerun：rerun 会删掉 gen-0..gen-(K-1) 历史清单+结构，
#                                  S6 累计数据集会丢帧；retry 保留它们并重生成新代清单
autozt -tt mlff -p Si start         # 判据检测到「代数不一致」→ 5/6/7/8 自动补生成/重跑/提交
```
...
`kl-mlff-cpu`/`phonon-mlff-cpu` 算**单一体系** κ/声子的专用势，不需要跨体系泛化。
...
- **2D 弛豫（S1）必须锁真空方向，且必须用 optcell 版 VASP**：三段式弛豫的 b/c 段是
  ...
  `PATIENCE` 不跳出循环、required 闸不影响 status 同一类）。**验收侧（mlff_relax.py 的
...
```bash
python mlff_finetune.py --name=<材料>_gen<K>_seed<S> --foundation_model=<基座.model> \
...
**本技能验收的是「势在谐波与准谐层面可信」；fc3 与 κ 的最终可信度要靠下游 `kl-mlff-*`
跑出与已知参考对照，本技能不替它背书。**
...
- `--loss` 传两次 / 硬编码 `--loss=weighted` 覆盖用户配置 → 应力项从未进损失，
  B0 偏 29% 却无人发现（mlff_finetune.py 去掉硬编码）。
...
处置：EPOCHS 按「真实收敛点 N × 1.5」设，别指望 PATIENCE 提前停。`mlff_finetune.py`
...
| `kl-mlff-cpu` / `phonon-mlff-cpu` | 本技能的 `<材料>_ft.model`（MACE_MODEL=文件名即可） | — |
...
改：`autozt -tt mlff -p <材料> -j <步骤> conf --set params.KEY=值`。
...
- MACE：CPU 用 `~/venvs/mace_cpu`（torch 2.7.1+cpu、mace-torch 0.3.16、phonopy 2.47、
  ...
  `CONDA_ENV` 指向 mace-gpu 环境 + 换 hpc 的 `submit_mlff.tpl`（A800 的 setting/a800
...
- 提交模板逻辑名：`submit_std_2d.tpl / submit_std_3d.tpl`（VASP，12 核由 step.conf
  `[submit] ntasks_per_node=12` 覆盖）、`submit_mlff.tpl`（step7/8）。
...
- [x] 目录自包含：gen_need 列的文件全部在 `skill/mlff/` 下（`dim_common.py /
      stepconf.py / relax_common.py / mlff_model.py / mlff_relax.py` 从 `_common` 拷入，
      其余为本技能自写）；无跨目录 import（relax_common 的 [MLFF] 补丁 import 同目录
      mlff_common）。
- [x] 依赖清单写在步骤级，每步列全提交模板逻辑名（submit_std_2d/3d、submit_mlff）。
...
- PATIENCE 单卡早停不生效（mace-torch 0.3.16 上游 bug，mlff_finetune 加版本警告）。
