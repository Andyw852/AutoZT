#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""mlff_common.py —— mlff（随机位移法 MLFF 训练）各步骤公共工具。
...
文件契约（全部在材料目录 <材料>/mlff/ 下，材料目录 = 各 gen 脚本的 cwd）：
...
# （此前硬编码 /public/home/<user> 导致换集群静默用错路径，报错只说文件不存在
# 不说"没配 CONDA_ENV"）。
DEFAULT_CONDA_SH = ""
DEFAULT_CONDA_ENV = ""
...
def resolve_dim(poscar, dimension="auto", vacuum_min=VACUUM_MIN):
    ...
    if dim == "0d":
        sys.exit("[ERROR] 检测到 0D 孤立体系（>=2 个真空方向）。mlff 只支持 "
...
SHARED_PARAM_SPEC = {
    ...
    "REPLAY_XYZ": (None, "str"),
    "E0S_MODE": ("estimated", "str"),        # estimated | json（见 mlff_finetune 注释）
    "N_COMMITTEE": (4, "int"),
