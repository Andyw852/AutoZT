#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_step9_publish.py —— mlff step9：发布（run: gen，仅 status==pass 才拷模型）。
...
def main():
    ...
    card = {
        ...
        "downstream_MACE_MODEL": ("/public/home/wangchao/software/mace/mlff_models/%s_ft.model" % mat)
    ...
    if status == "pass":
        ...
        dst_dir = Path(cv["MACE_MODEL_DIR"] or
                       "/public/home/wangchao/software/mace/mlff_models")
        ...
        note = ("status=%s：还没到发布阶段。按 README 手动推进下一代："
                "autozt conf --set params.GENERATION=%d → tf -j 4 rerun → autozt start。"
                % (status, gen + 1))
