...
- **ASE 用法**：
  ...
  ```
- **fc2/fc3 管线**：calorine 内置 `calorine.tools.get_force_constants(structure, supercell_matrix, ...)`（基于 phonopy 有限位移，跑 GPUMD，返回 Phonopy 对象，可带 fc3、非解析项修正）；GPUMD 自带 compute_phonon。**NEP 是无预训练通用模型**：须用随机位移法（mlff 同款思路）自训，训练好之后声子/热导口碑极好（描述子势平滑、能量-梯度保守、速度快）。
