...
> v1.0 配套。目标：**加一个技能 = 复制一个目录 + 改 5 处**，不用改 phonoagent 核心、
> 不用改 phonoagent.yaml、不用问作者。

## 0. 先跑一遍看效果（3 分钟）

```bash
cd ~/software/PhonoAgent-v1.0                  # 软件仓库根
...
python3 bin/phonoagent skills                        # ② 零配置自动发现：my-skill 出现在表里
python3 bin/phonoagent schema my-skill               # ③ 看它的自描述（吃什么/吐什么/能接谁/怎么纠错）
```

第 ② 步是关键：**phonoagent 只认 `skill/<名字>/skill.yaml`，放进目录即被自动发现**，
不需要在 phonoagent.yaml 里登记任何东西。
...
| 2 | `skill.yaml` 的 `name` / `desc` / `version` | `name` = 全局唯一技能 key = 以后 `phonoagent -tt <name>` 用的名字 |
| 3 | `skill.yaml` 的 `steps` | 你的流水线：每步一个 gen 脚本 + 一个判据；`seq` 决定顺序 |
| 4 | `gen_step1_summary.py` | 换成你自己的生成/后处理脚本（见第 3 节） |
| 5 | `io_schema` / `flow` / `corrections` 三段 | 自描述；`phonoagent schema <技能>` 会校验，写错会给出 [警告]/[错误] |
...
| `steps[].label` | 显示短名，`phonoagent -j S1_summary` 用它 |
...
| `steps[].done_marker` | run: gen 步的产出文件名（phonoagent 据此判完成并自动拉回） |
...
| `defaults.hpc` / `work_dir` | 站点相关缺省值（用户在 phonoagent.yaml / 项目配置里覆盖） |

## 3. gen 脚本的契约（★ 最容易踩的地方）

- phonoagent 把脚本推到远端，**cwd = 该步骤目录**（`材料/<技能>/<步骤名>/`）；
- 脚本自己写 `INCAR/KPOINTS/POSCAR/submit.sh`（要提交 SLURM 的步）；
- 需要模板/公共库时写进 `gen_need`，phonoagent 会一起推过去；
- `gen` 里可以写 `{mat}` / `{matdir}` / `{root}` / `{step}` / `{tt}` 占位符，phonoagent 会替换；
- 参数从 `step.conf` 读（公共池 `stepconf.py`，见 opt-dft-cpu 的用法），
  用户改参数走 `phonoagent -p X -j <步骤> conf --set params.KEY=值`。
...
- `io_schema` —— 这个技能**吃什么、吐什么、有哪些旋钮**。写了以后：
  新人（和 AI）不用读代码就知道怎么用、产物在哪；参数写错会在 `phonoagent schema` 里报出来。
...
- `corrections` —— **这类失败该怎么纠**，引用 `skill/_common/_corrections/` 里的
  handler（见那份 README）。写了以后 `phonoagent diagnose` / `phonoagent correct` 会给具体建议。

校验：`python3 bin/phonoagent schema <技能>`（`--strict` 有错误时返回非零，可进 CI）。

## 5. 上线前自检清单

```bash
python3 bin/phonoagent skills                 # ① 被发现了？（版本/步骤数/自描述列）
python3 bin/phonoagent schema <技能> --strict  # ② 自描述没写错？
python3 bin/phonoagent -tt <技能> init         # ③ 能在项目里初始化配置段？
python3 bin/phonoagent -tt <技能> -p <材料> -j <步骤> init   # ④ 输入生成对不对（只生成不提交）
python3 bin/phonoagent -tt <技能> -p <材料> -j <步骤> start  # ⑤ 确认无误再提交
```
