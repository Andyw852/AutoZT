...
`templates/mlff/` **不在这条链上**（它不是步骤名）。所以模型放这里 autozt 不会自动推。
...
```bash
# 本地：模型下载/训练完丢进本目录
ls skill/kl-mlff-cpu/templates/mlff/
#   mace-mpa-0-medium.model
#   my-finetuned-C60.model

# 一次性推到超算（本目录的 push_model.sh 就干这个）
bash skill/kl-mlff-cpu/templates/mlff/push_model.sh jzzn /public/home/wangchao/software/mlff_models
```

然后在 step.conf 里指名字（路径由 `MACE_MODEL_DIR` 提供）：

```
autozt -tt kl-mlff-cpu -p <材料> conf --set params.MACE_MODEL=mace-mpa-0-medium.model
autozt -tt kl-mlff-cpu -p <材料> conf --set params.MACE_MODEL_DIR=/public/home/wangchao/software/mlff_models
```
...
把 `.model` 拷进 `templates/step2_disp_force/` 和 `templates/step1_mlff_relax/`，
...
- `step1_mlff_relax/relax_summary.json` 和 `step2_disp_force/forces_summary.json`
