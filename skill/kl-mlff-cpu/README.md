# kl-mlff-cpu —— 晶格热导率（MACE，CPU 取力）

引擎、物理说明、安装、虚频排查在 `skill/_common/mlff/README.md`（两版共用）。
本文只写 CPU 版特有的东西。

```
autozt -tt kl-mlff-cpu -p <材料> init
autozt -tt kl-mlff-cpu -p <材料> start
```
...
```bash
autozt -tt kl-mlff-cpu -p <材料> -j step2_disp_force conf --set params.METHOD=random
autozt -tt kl-mlff-cpu -p <材料> -j step2_disp_force conf --set params.N_RANDOM=120   # 固定帧数；不设=auto
autozt -tt kl-mlff-cpu -p <材料> -j step2_disp_force conf --set params.FC2_SUPERCELL="5 5 5"
```
...
`submit_mlff.tpl` 是**单进程多线程**（`ntasks-per-node=1` + `cpus-per-task=48`）。
MACE 靠 torch 线程并行，`mpirun -np N` 只会起 N 份互相抢核的独立进程，把同一件事
算 N 遍。

MACE 在 CPU 上的线程扩展性一般 16~32 核就饱和，整节点 48 核未必快多少，还要多等队列。
第一次跑完看 `mlff_forces.log` 里的「帧/s」，再决定下次申请多少。

`CKPT=10` 意味着每 10 帧落一次盘。作业被墙钟砍掉后 `autozt -tt kl-mlff-cpu -p <材料> retry`
...
看 `templates/mlff/README.md`。一句话：模型在超算上放一份（`push_model.sh`），
...
- **运行**：`SOLVER=shengbte` + step4 step.conf 填 `SHENGBTE_EXE`（按集群绝对路径）：
  - jzzn = `/public/home/<user>/software/sousaw-shengbte-aocl/ShengBTE`（CPU/AOCL）
  - 3090 = `/home/<user>/software/AutoZT/shengbte-gpu/ShengBTE`（GPU fork，~4.6x）
...
- **限制**：shengbte 只支持单套网格（`MESH_SCAN` 留空）；NAC 用 step3 的 BORN（MACE 势
  ...
  （`convergence=F`），迭代 CONV 内存/耗时更大，kl-mlff 链先走 RTA。
