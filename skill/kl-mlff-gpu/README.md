# kl-mlff-gpu —— 晶格热导率（MACE，GPU 取力）

引擎、物理说明、安装、虚频排查在 `skill/_common/mlff/README.md`（两版共用）。
本文只写 GPU 版特有的东西。

```
autozt -tt kl-mlff-gpu -p <材料> init
autozt -tt kl-mlff-gpu -p <材料> start
```
...
`mlff_model.pick_device()` 在 `DEVICE=cuda` 且 `torch.cuda.is_available()` 为假时直接
退出，`submit_mlff.tpl` 里还有一道独立的前置检查。这是故意的：在 GPU 队列上退回 CPU，
...
```bash
autozt -tt kl-mlff-gpu -p <材料> -j step2_disp_force conf --set params.FC2_SUPERCELL="6 6 6"
autozt -tt kl-mlff-gpu -p <材料> -j step2_disp_force conf --set params.MIN_SC_LEN=22
autozt -tt kl-mlff-gpu -p <材料> -j step4_kappa conf --set params.MESH_SCAN="16 16 16; 20 20 20; 24 24 24"
```

默认值：`MIN_SC_LEN=12`、`KAPPA_MESH=24 24 24`、`CKPT=50`、`N_RANDOM=200`，
`[submit]` 里 `partition=gpu`、`gres=gpu:1`。队列名按你们集群改。

多卡不会更快——`mlff_forces.py` 没做数据并行，申请多卡只是浪费。

## 模型

看 `templates/mlff/README.md`。GPU 版显存够的话可以考虑 `mace-mp:large`，
推理时间增加不到一倍，精度是白拿的。
