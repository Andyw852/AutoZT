...
（`stepconf.py` 解析、三层合并）。物理链对齐参考引擎 `lattice_kappa.py`，拆成 phonoagent 异步分步。
...
4. **声子拟合步**：step5 独立拟合力常数——findiff 用 `--sym-fc`，alm 用 `--alm`（压缩感知
   回归），再出声子谱做虚频闸。
5. **参数模板化**：每步关键参数写进 `templates/<步骤名>/step.conf`，gen 脚本只读这份 phonoagent
   合成的文件，不再在脚本顶部写死。
...
S3_nac 是可选步组：默认开；关掉在项目配置写 `nac: false`，phonoagent 直接不注入 S3，S5/S6 因无
BORN 自然不加 NAC。

S1 的起始结构建议复用其它链已优化好的 CONTCAR：`python3 reuse_structure.py <材料名>`
（脚本在 `skill/kl-dft-cpu/`），按 ke → opt → band → elastic 顺序找候选 CONTCAR
（候选目录见 `_STEP1_CANDS`），命中就复制成**材料根 POSCAR**（phonoagent 的 gen 一律从材料根取
...
## 安装
把整个 `kl-dft-cpu/` 放到 `~/software/PhonoAgent/skill/kl-dft-cpu/`，然后：
```
phonoagent skills            # 应看到 kl-dft-cpu 0.2 / 6 步 / 启用
```

## 用法
```
phonoagent -tt kl-dft-cpu -p <材料> init      # 生成 材料/kl-dft-cpu/project_setting/（含 templates/<步骤>/）
phonoagent -tt kl-dft-cpu -p <材料>           # 看步骤链
# 按序推进各步（与你 band-dft-cpu/elastic-dft-cpu 一致的 advance/run 流程）
```

## 改参数（不要改脚本，改 step.conf）
```
phonoagent -tt kl-dft-cpu -p <材料> -j <步骤> conf                    # 看某步最终生效值 + 来源
phonoagent -tt kl-dft-cpu -p <材料> -j step4_disp conf --set params.METHOD=alm
phonoagent -tt kl-dft-cpu -p <材料> -j step4_disp conf --set params.SUPERCELL="3 3 3"
phonoagent -tt kl-dft-cpu -p <材料> -j step6_kappa conf --set params.KAPPA_MESH="24 24 24"
```
...
- **3090（GPU 版）**：`/home/wangchaoyue852/software/PhonoAgent/shengbte-gpu/ShengBTE`
  —— 即 **2020 HPC Asia 论文 CUDA fork**（仓库 buaa-hipo/ShengBTE-Multiplatform，克隆在
  `~/software/PhonoAgent/shengbte-gpu/`，含 Src-gpu 源码）。已修复其 dGamma 未清零 bug，**phonopy fc2
  输入数值与官方 CPU 一致**（15³ RTA 8 温度点一致到 4~6 位有效数字），RTX3090 实测 ~4.6× 加速。
  运行需 `~/software/PhonoAgent/shengbte-gpu/run_env.sh` 设 NVHPC/OpenMPI/CUDA12.4/spglib/OpenBLAS 环境，
...
- **3090（官方 v1.3 CPU，可选）**：`/home/wangchaoyue852/software/PhonoAgent/fourphonon-v13/bin_cpu`
...
**已固化的 multi-GPU 版**（源码 + patch 在 ~/software/PhonoAgent/fourphonon-v13-test/）：
...
`task_types` 的同级字符串键，`phonoagent -tt <任意>` 会抛
`dictionary update sequence element #0 has length 1; 2 is required`。把那一行改成 4 空格缩进即可。
（本次在干净仓库里跑 `phonoagent -tt kl-dft-cpu` 无此报错，印证问题在该缩进。）
