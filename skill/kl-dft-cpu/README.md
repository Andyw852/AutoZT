...
（`stepconf.py` 解析、三层合并）。物理链对齐参考引擎 `lattice_kappa.py`，拆成 autozt 异步分步。
...
4. **声子拟合步**：step5 独立拟合力常数——findiff 用 `--sym-fc`，alm 用 `--alm`（压缩感知
   回归），再出声子谱做虚频闸。
5. **参数模板化**：每步关键参数写进 `templates/<步骤名>/step.conf`，gen 脚本只读这份 autozt
   合成的文件，不再在脚本顶部写死。
...
（候选目录见 `_STEP1_CANDS`），命中就复制成**材料根 POSCAR**（autozt 的 gen 一律从材料根取
...
## 安装
把整个 `kl-dft-cpu/` 放到 `~/software/AutoZT/skill/kl-dft-cpu/`，然后：
```
autozt skills            # 应看到 kl-dft-cpu 0.2 / 6 步 / 启用
```

## 用法
```
autozt -tt kl-dft-cpu -p <材料> init      # 生成 材料/kl-dft-cpu/project_setting/（含 templates/<步骤>/）
autozt -tt kl-dft-cpu -p <材料>           # 看步骤链
# 按序推进各步（与你 band-dft-cpu/elastic-dft-cpu 一致的 advance/run 流程）
```

## 改参数（不要改脚本，改 step.conf）
```
autozt -tt kl-dft-cpu -p <材料> -j <步骤> conf                    # 看某步最终生效值 + 来源
autozt -tt kl-dft-cpu -p <材料> -j step4_disp conf --set params.METHOD=alm
autozt -tt kl-dft-cpu -p <材料> -j step4_disp conf --set params.SUPERCELL="3 3 3"
autozt -tt kl-dft-cpu -p <材料> -j step6_kappa conf --set params.KAPPA_MESH="24 24 24"
```
...
- **jzzn（CPU 版）**：`/public/home/.../software/sousaw-shengbte-aocl/ShengBTE`
  （官方 2024 内核 + AOCL，提交模板 submit_shengbte.tpl 已配套 module gcc/14.1 + openmpi/4.0.1 + aocl-gcc）。
- **3090（GPU 版）**：`/home/user_3090/software/AutoZT/shengbte-gpu/ShengBTE`
  —— 即 **2020 HPC Asia 论文 CUDA fork**（仓库 buaa-hipo/ShengBTE-Multiplatform，克隆在
  `~/software/AutoZT/shengbte-gpu/`，含 Src-gpu 源码）。已修复其 dGamma 未清零 bug，**phonopy fc2
  输入数值与官方 CPU 一致**（15³ RTA 8 温度点一致到 4~6 位有效数字），RTX3090 实测 ~4.6× 加速。
  运行需 `~/software/AutoZT/shengbte-gpu/run_env.sh` 设 NVHPC/OpenMPI/CUDA12.4/spglib/OpenBLAS 环境，
...
- **3090（官方 v1.3 CPU，可选）**：`/home/user_3090/software/AutoZT/fourphonon-v13/bin_cpu`
...
**已固化的 multi-GPU 版**（源码 + patch 在 ~/software/AutoZT/fourphonon-v13-test/）：
