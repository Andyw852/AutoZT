...
def build_extract(factor, meta, thick2d=None, plan=None, primary=None):
    ...
    body = [
        ...
        "# ---- imag_policy（唯一真源）：透传 S5 的虚频结论；warn 放行但要注明 κ 的可能偏差 ----",
        "try:",
        "    _ps = json.load(open('../step5_fc/phonon_summary.json', encoding='utf-8'))",
        "    _map = {'stability_verdict': 'stability_verdict', 'imag_class': 'imag_class',",
        "            'min_freq_THz': 'min_freq_THz', 'min_freq_cm1': 'min_freq_cm1',",
        "            'q_at_min_frac': 'q_at_min_frac', 'n_neg_qpoints': 'n_neg_qpoints',",
        "            'policy_version': 'imag_policy_version'}",
        "    for _k, _dst in _map.items():",
        "        if _ps.get(_k) is not None: d[_dst] = _ps.get(_k)",
        "    if str(_ps.get('stability_verdict')) == 'warn':",
        "        d['warn_note'] = ('imag_policy=warn（近 Γ 声学支软化）：phono3py 会排除负频模，'",
        "                          'κ 可能低估近 Γ ZA 的贡献。建议收紧收敛参数后复算：'",
        "                          '真空层厚度 / 面内超胞大小 / 残余应力 / k 网格'",
        "                          '（Lin et al. 2022：近 Γ ZA 小虚频多数可通过收紧收敛消除）')",
        "except Exception as _e:",
        "    d['imag_policy_note'] = '未取到 S5 的 phonon_summary.json：%s' % _e",
        ...
        "    try:",
        "        import numpy as _np",
        "        # N 取网格里最大的分量（=面内 N）。不能取 split()[0]：phono3py 原胞重排后",
        "        #   2D 网格常是 '1 N N'（真空轴在 a1），第一个数恒为 1 -> 1/N 全相同 -> 外推退化",
