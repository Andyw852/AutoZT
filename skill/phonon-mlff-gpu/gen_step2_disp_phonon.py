#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_step2_disp_phonon.py —— 超胞 + 随机位移帧数 + submit.sh（phonon-mlff-cpu S2）。

只算 2 阶（声子）：帧数 N 用 ALM 数 2 阶自由力常数反推
（N = max(10, ceil(nfree_fc2/(3*N_sc)) × OVERSAMPLE)），位移振幅 0.01 Å。
计算节点作业里由 mlff_forces_phonon.py 生成位移 + 取力。
"""
import shutil
import sys
...
def main():
    ...
    for f in ("mlff_forces_phonon.py", "mlff_model.py", "mc_rattle.py"):
        if not (here / f).is_file():
            sys.exit("[ERROR] 缺 %s —— 本步 gen_need 里漏了它？" % f)
        shutil.copyfile(str(here / f), str(out / f))

    cmd = ("python mlff_forces_phonon.py --model %s --model-dir '%s' "
    ...
    tpl = kc.resolve_submit(here, "submit_mlff_relax")
