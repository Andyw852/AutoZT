#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_step5b_uniform_full.py —— AMSET **全网格**波函数用的密网格自洽（step3b_uniform_full）。

与 gen_step5_uniform.py（S3_uniform）**逐行等价，只有三处不同**：
  1. OUTDIR_NAME / STEP_LABEL 换成 step3b_uniform_full / S3b_uniformfull；
...
# =========================== 可改参数区 ===========================
OUTDIR_NAME  = "step3b_uniform_full"
...
def main():
    ...
    if dim is None:
        ...
        require_dim(dim, ('2d', '3d'), "step3b_uniform_full",
    ...
    if _tot > int(UNIFORM_NMAX):
        sys.exit("[ERROR] step3b_uniform_full 网格 %dx%dx%d = %d 点，超过 UNIFORM_NMAX=%s。"
    ...
    print("[OK] ISYM = -1（全网格模板）：VASP 将计算全部 %d 个 k 点，"
          "AMSET 会直接走 from_data、不做去对称化" % _nfull)
    _ref = cwd / "step3_uniform" / "KPOINTS"
    if _ref.is_file():
        try:
        ...
        if _rn and list(_need[:3]) != list(_rn):
            ...
            print("[WARN] 全网格诊断的前提是'只改 ISYM'。要复现同一网格，请在")
            print("[WARN] project_setting/templates/step3b_uniform_full/step.conf 里")
            print("[WARN] 写上与 step3_uniform 相同的 VACUUM_KZ_MIN / DK_MAX。")
            print("[WARN] " + "=" * 60)
        elif _rn:
            print("[OK] 网格与 step3_uniform 一致（%dx%dx%d）—— 与它只差 ISYM" % (_rn[0], _rn[1], _rn[2]))

    kc.vaspkit_potcar(out, VASPKIT_EXE)

    encut = MANUAL_ENCUT or kc.encut_from_potcar(out / "POTCAR", ENCUT_FACTOR)
    tpl = Path(__file__).resolve().parent / ("incar_uniform_full_%s.tpl" % dim)
    if not tpl.is_file():
