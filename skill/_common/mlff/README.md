# _common/mlff —— MACE 晶格热导率引擎（kl-mlff-gpu / kl-mlff-cpu 共用）

两个技能 `kl-mlff-gpu` 和 `kl-mlff-cpu` 共用这一份代码。技能目录里只有 `skill.yaml`
...
```bash
cp skill/_common/mlff/*.py skill/kl-mlff-gpu/     # 拷进去 = 覆盖公共池
rm skill/kl-mlff-gpu/*.py                          # 删掉 = 回到共用
```
...
| 文件 | 在哪跑 | 干什么 |
|---|---|---|
| `gen_step1_mlff_relax.py` | 登录节点 | 建目录、接 POSCAR、调 `mlff_relax.py` |
...
| `mlff_relax.py` | conda 环境 | ASE + FrechetCellFilter + FixSymmetry 弛豫 |
| `mlff_forces.py` | conda 环境（计算节点） | 遍历位移超胞取力，断点续算 |
| `mlff_model.py` | conda 环境 | 模型定位 + calculator 构造 + 设备判定 |
| `klmlff_common.py` | 两边 | 维度/超胞/模板渲染/conda 子进程 |
...
```bash
# 解包（会把引擎放进 _common/mlff/，两个技能各一个目录）
tar xzf klmlff2.tar.gz -C ~/software/AutoZT/
autozt skills            # 应看到 kl-mlff-gpu / kl-mlff-cpu 各 4 步、启用
```
...
1. `step1_mlff_relax/relax_summary.json` 的 `max_force_eV_per_A` 真的压到
...
`MACE_MODEL_DIR` 由 `setting/<hpc>.yaml` 的 `conda_sh`/`conda_env`/`mlff_model_dir`
...
- **没做批量推理**。`mlff_forces.py` 是逐个结构调 ASE calculator。GPU 上做 batch
