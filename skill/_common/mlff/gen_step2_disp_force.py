...
分析）→ 报位移数 → 写 submit.sh。
计算节点那一半：submit.sh 跑 mlff_forces.py，串行遍历全部位移超胞取力。
...
SPEC = {
    ...
    "KAPPA_MESH": ("24 24 24", "str"),     # 写进 klmlff_params 供 step4
...
def main():
    ...
    here = Path(__file__).resolve().parent
    for f in ("mlff_forces.py", "mlff_model.py"):
        if not (here / f).is_file():
            sys.exit("[ERROR] 缺 %s —— 本步 gen_need 里漏了它？" % f)
        shutil.copyfile(str(here / f), str(out / f))

    cmd = ("python mlff_forces.py --disp-yaml phono3py_disp.yaml "
    ...
    tpl = kc.resolve_submit(here, "submit_mlff")
