#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""plot_phonon_band.py —— MACE 声子谱绘图（phonon-mlff-cpu/gpu、kl-mlff-cpu/gpu 共用）。
...
的兄弟层），band yaml 在下列候选位置探测（与 kl-dft-cpu S5.1 的 _locate 同思路）：
  <技能>/step3_phonon/band-dft-cpu.yaml   (phonon-mlff-*)
  <技能>/step3_fc/band-dft-cpu.yaml       (kl-mlff-*)
...
# 步骤目录候选（相对 CWD 的兄弟步骤）：phonon-mlff 用 step3_phonon，kl-mlff 用 step3_fc
