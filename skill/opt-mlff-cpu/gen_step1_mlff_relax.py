#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_step1_mlff_relax.py —— MACE 结构优化（step1_mlff_relax），submit 模式。

opt-mlff-cpu 专用（技能目录优先于 _common/mlff）：本步只准备输入 + 写 submit.sh，
MACE 弛豫放到计算节点作业里跑（不阻塞登录节点、批量 138 个材料也能秒级提交）。
产出（作业跑完后）：step1_mlff_relax/{CONTCAR, relax_summary.json, workflow_method.txt}
...
OUTDIR = "step1_mlff_relax"
STEP = "step1_mlff_relax"
...
def main():
    ...
    require_dim(dim, ("2d", "3d"), "step1_mlff_relax",
