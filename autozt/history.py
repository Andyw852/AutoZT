...
为什么要它：现在只有"此刻的状态"（autozt list/summary）和"与上次比变了什么"
（autozt summary --diff）。于是
...
记录时机：任何**真正采集**（autozt list/summary/status/json/monitor）之后自动记录；
...
def cmd_history(cfg, proj=None, tt=None, since=None, last_n=40, json_out=False):
    ...
    记录是**自动**的——任何一次真正采集（autozt list/summary/status/monitor）之后
    ...
    if total == 0:
        ...
        print("  autozt list --refresh      # 或 autozt summary --refresh / autozt status / autozt monitor")
