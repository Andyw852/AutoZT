...
本模块只做三件事，**纯函数、不读盘、不连超算、不改状态**：
  ...
  3. 把三段渲染成 autozt schema 的控制台文本 / 结构化 dict。

调用方：
  · autozt/bootstrap.py  发现技能时算一份轻量 issues（autozt skills 显示）
  · autozt/cli.py        autozt schema [<技能>] 子命令（cmd_schema）
...
def validate_skill_spec(key, man, skel=None, known_skills=None, handler_names=None):
    ...
    分级：[错误] 会让 autozt schema --strict 失败（结构不合法）；[警告] 只提示（多半是笔误）。

    handler_names=None 时跳过"corrections 名是否已注册"这一条（tf 启动期的
    轻量校验用；autozt schema 会带上真正的 handler 注册表做完整校验）。"""
...
# =============================================================================
# 抽取 + 统计（autozt skills 一行摘要 / autozt schema 表头都用）
# =============================================================================
def spec_of(man):
...
def render_schema(key, spec, skel=None, issues=None, extra_head=None):
    """把单个技能的三段渲染成控制台文本（autozt schema <技能>）。"""
    skel = skel or {}
    spec = spec or {}
    st = spec_stats(spec)
    L = []
...
def render_schema_table(rows):
    """autozt schema（不带技能名）：全部技能一览。rows = [(key, skel, spec, issues)]"""
    L = []
    ...
    L.append("看单个技能：autozt schema <技能名>     机器可读：autozt schema --json")
    return "\n".join(L)


def schema_dict(key, skel, spec, issues):
    """autozt schema --json 的单技能结构（稳定字段名，供工具/AI 判读）。"""
    return {
        "skill": key,
...
def cmd_schema(cfg, tt=None, json_out=False, strict=False):
    """autozt schema [<技能>] —— 打印技能的自描述（io_schema / flow / corrections）+ 校验。
    ...
    try:                      # 纠错 handler 注册表（可能有多个候选目录，取并集）
        from autozt import correction_handler_names
        ...
        if issues:
            ...
            if errs or warns:
                body.append(_pad(" Validation: %d errors / %d warnings "
                                 "(see autozt schema %s)" % (len(errs), len(warns), key), inner))
